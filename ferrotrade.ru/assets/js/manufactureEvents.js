       ...
       success: function(data, textStatus)
       {
          if (data){
             ...
             $("#content").unbind('click').delegate('#regMany', 'click', function(){
                ...
                $.ajax({
                   type: 'POST',
                   url: `${curHost}/api/manufacture/add`,
                   contentType: "application/json; charset=utf-8",
...
function bind_preTransfer(){
   ...
   $('#content').unbind('click').delegate('#preTransfer', 'click', function(){
      ...
      $.ajax({
         type: 'POST',
         url: `${curHost}/api/manufacture/departurePreProcess`,
         dataType: 'json',
         contentType: 'application/json; charset=utf-8',
...
function bind_transfer(cards_id){
   ...
   $('#content').unbind('click').delegate('#transfer', 'click', function(){
      ...
      $.ajax({
         type: 'POST',
         url: `${curHost}/api/manufacture/transfer`,
         contentType: "application/json; charset=utf-8",
         data: JSON.stringify(object),
         success: function(data, textStatus)
         {
            window.open(`${curHost}/api/transferDocs/${data}`, "_blank");
            console.log(data);
            $('#content').html('');
         },
         error: function(data, textStatus)
...
$('.addManyForm').delegate('.close-block-regMany', 'click', function(){
    ...
    $.ajax({
       ...
       success: function(data, textStatus)
       {
          ...
          `${curHost}/api/code/${id}` + `' />
           ...
           <p style='font-size: 20px; margin: 0'>Масса: ${data.mass.toFixed(2)}       ОТК1</p></body></html>
          `;
          console.log(str);
       },
       error: function(data, textStatus){
 ...
 function PackageToPrint(id)
 {
    ...
    $.ajax({
       type: 'GET',
       url: `${curHost}/api/manufacture/getById/${id}`,
       dataType: 'json',
       async: false,
       success: function(data, textStatus)
       {
